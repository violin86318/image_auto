...
def extract_youtube_embed_url(url: str) -> str:
    """从 YouTube URL 提取 embed URL，返回空字符串表示非 YouTube"""
    if not url:
        return ""
    parsed = urlparse(url)
    video_id = ""
...
def generate_html(analysis, video_path=None):
    ...
    video_input = meta.get("video_input", "")
    ...
    # 判断视频来源：YouTube iframe 或本地 base64
    youtube_embed_url = extract_youtube_embed_url(video_input)
    video_b64_url = ""
    if not youtube_embed_url and video_path and os.path.isfile(video_path):
...
.player-wrap video,.player-wrap iframe{{flex:1;min-width:300px;max-width:640px;border-radius:8px;background:#000;border:none;aspect-ratio:16/9}}
...
{'<iframe id="ytPlayer" src="' + youtube_embed_url + '" allowfullscreen></iframe>' if youtube_embed_url else '<video id="mainVideo" controls><source src="' + video_b64_url + '" type="video/mp4"><track id="metaTrack" kind="metadata" default></video>'}
...
// Video sync — 支持本地 <video> 和 YouTube <iframe>
(function(){{
...
function highlightScene(ct){{
let activeIndex = -1;
scenes.forEach((sc, idx)=>{{
const s=parseTime(sc.dataset.start||'0');
const e=parseTime(sc.dataset.end||'9999');
if(ct>=s&&ct<e){{activeIndex = idx; sc.classList.add('highlight');}}
else{{sc.classList.remove('highlight');}}
}});

if(activeIndex !== -1 && activeIndex !== lastSceneIndex){{
lastSceneIndex = activeIndex;
if(panel) panel.innerHTML = '<div class="active-scene">' + scenes[activeIndex].innerHTML + '</div>';
}}
}}

// 本地视频同步
if(localVideo&&scenes.length){{
localVideo.addEventListener('timeupdate',function(){{highlightScene(localVideo.currentTime);}});
}}

// YouTube iframe 同步 (通过 YouTube IFrame API)
if(ytFrame){{
var ytPlayer=null;
...
events:{{
  'onReady':function(){{ highlightScene(0); }},
  'onStateChange':function(e){{
    if(e.data === YT.PlayerState.PLAYING && !window.ytInterval){{
      window.ytInterval = setInterval(function(){{if(ytPlayer&&ytPlayer.getCurrentTime)highlightScene(ytPlayer.getCurrentTime());}},500);
    }} else if(e.data !== YT.PlayerState.PLAYING && window.ytInterval) {{
      clearInterval(window.ytInterval); window.ytInterval = null;
    }}
  }}
}}
}});
...
if(v){{v.currentTime=t;v.play();return;}}
// YouTube seek
const yt=document.getElementById('ytPlayer');
if(yt&&window.YT){{try{{var p=YT.get('ytPlayer');if(p&&p.seekTo){{p.seekTo(t,true);}}}}catch(e){{}}}}
}};
</script></body></html>'''
    return html
